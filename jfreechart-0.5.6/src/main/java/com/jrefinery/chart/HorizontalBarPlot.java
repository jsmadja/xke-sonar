 ...
 * MA 02111-1307, USA.
 *
 * $Id: HorizontalBarPlot.java,v 1.1.1.1.2.2 2000/11/14 21:29:06 dgilbert Exp $
 */
...
public class HorizontalBarPlot extends BarPlot implements HorizontalValuePlot {
   ...
   */
  public HorizontalBarPlot(JFreeChart chart,
   ...
   */
  public HorizontalBarPlot(JFreeChart chart, Axis horizontalAxis, Axis verticalAxis)
         throws AxisNotCompatibleException {
  ...
  double calculateBarWidth(Rectangle2D plotArea) {
    ...
    double usable = plotArea.getHeight() - introGap - trailGap
  ...
  public void draw(Graphics2D g2, Rectangle2D drawArea) {
    ...
    // we can cast the axes because HorizontalBarPlot enforces support of these interfaces
    VerticalAxis va = getVerticalAxis();
    HorizontalAxis ha = getHorizontalAxis();

    double w = va.reserveWidth(g2, this, drawArea);
    Rectangle2D hAxisArea = ha.reserveAxisArea(g2, this, drawArea, w);

    // compute the plot area
    Rectangle2D plotArea = new Rectangle2D.Double(drawArea.getX()+w,
                                                  drawArea.getY(),
                                                  drawArea.getWidth()-w,
      ...
      while (iterator.hasNext()) {
        ...
        for (int seriesIndex=0; seriesIndex<seriesCount; seriesIndex++) {
          ...
          if (barVisible) {
            ...
            bars.add(new Bar(base, barX, top-base, barWidth,
